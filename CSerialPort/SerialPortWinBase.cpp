...
bool CSerialPortWinBase::open()
{
	...
	_tcscpy_s(szPort, CA2T(m_portName.c_str()));
	...
	if (!isOpened())
	{
		...
		m_handle = CreateFile(szPort,						// communication port string (COMX)
		...
		else
		{
			...
			case ERROR_FILE_NOT_FOUND:
			{
										 ...
										 MessageBox(NULL, _T("打开串口失败!!!,或其它错误。\r\n请选择正确的串口或该串口被占用"), _T("错误提示"), MB_OK);
										 break;
			}
				//串口拒绝访问
